...
export const getFollowingService = async (userId) => {
  const user = await User.findById(userId).populate("following", "_id name username email image");
...
export const getFollowersService = async (userId) => {
  const user = await User.findById(userId).populate("followers", "_id name username email image");
  if (!user) throw new Error("User not found");
